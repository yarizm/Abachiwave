...
3. **时长缩放补止音（×0.85 是该族的上限）。** 与 Basic Pitch 不同，YourMT3 的时长误差是**有偏**的：
   ...
   逐音符的声学修正也试过：用 pYIN 的逐帧有声判定，把音符止音裁剪到其跨度内最后一个有声帧。
   单独使用严重变差（开发集 0.397 vs 0.466），且尾部容差 0 帧最优——说明**有声边界不是标注的
   音符止音**，标注者把清音尾与衰减计入音符，而周期性结束得更早。裁剪叠加 ×0.90 在开发集为
   0.460、留出集 0.464。留出集虽高出现行方案 0.023，但开发集低 0.006，按"开发集选参、留出集
   验证"的规则不予采用——反过来用留出集挑选正是第 9 节方法论要拦截的错误，且两者在开发集上
   本就落在噪声内。

### 12.4 部署成本

第 12.2 节的原型数字是**主机进程、且只跑模型**测得的。上线形态是容器 + 完整三部件管线，
成本明显更高，两者必须分开记：

| 指标 | Basic Pitch 0.4.0 | YourMT3 原型（主机、仅模型） | YourMT3 sidecar（容器、完整管线） |
| --- | ---: | ---: | ---: |
| 中位 RTF | 0.023 | 0.824 | **1.50**（P95 2.12） |
| 中位时延 | 0.44 s | 16.7 s | 27.9 s（P95 53.8 s） |
| 峰值内存 | 720–770 MiB | 2205 MB（主机进程） | 1.42 GiB（容器稳态） |
| 镜像体积 | — | — | 507 MB（CPU-only wheel） |
| 权重 | 内置 | 536 MB，挂载在镜像外 | 同左，启动时校验 SHA-256 |

在 33.2 s 的样本上拆解：仅模型 35.0 s（RTF 1.05），完整管线 67.8 s（RTF 2.04）——**pYIN 占了约一半
时间**。容器内模型比主机慢约 28%。首次请求 96 s 含冷启动，稳态以第二次为准。

因此实际代价是比 Basic Pitch 慢**约 65 倍**，不是原型阶段估计的 36 倍（上面 40 条的中位 1.50
才是可用数字；单条 33 s 样本上测得的 2.04 偏高）。上传上限 25 MB 对应约
136 s 音频，按 RTF 2.0 约需 273 s，`YOURMT3_TIMEOUT_SECONDS` 默认 600 s 留有两倍余量。
镜像 507 MB 则显著小于按 CUDA 版 PyTorch 的估计。

**上线形态已复核。** 用部署好的 sidecar 对同一 40 条重跑并用 `support.score_external_midi` 打分：
development 0.692/0.467、holdout 0.623/0.441、overall 0.675/0.460，与第 12.2 节原型数字逐位一致
（开发集 offset 差 0.001，来自 pYIN 重采样路径的舍入）。也就是说部署的管线与被测的管线是同一个。
