...
3. **时长缩放补止音。** 与 Basic Pitch 不同，YourMT3 的时长误差是**有偏**的：
...
- **未测组合。** 在 YourMT3 检测到、Basic Pitch 也有命中的音符上改用 Basic Pitch 音高
  （准确率 87.6–92.0% 高于 pYIN），可能进一步改善；但 Basic Pitch 只覆盖约 60% 的起音，
  且会把管线增至四个部件，尚未评估。
