...
代价（以上线形态计，非原型）：容器内完整管线中位 RTF 约 2.0，比 Basic Pitch 的 0.023 慢**约 87 倍**；
其中 pYIN 约占一半时间，容器较主机再慢约 28%。容器稳态内存 1.42 GiB，镜像 507 MB，权重 536 MB
挂载在镜像外并在启动时校验 SHA-256。原型阶段记录的 RTF 0.824 是主机上只跑模型的数字。

与已发表基线同口径（全集 40 条）对照：组合方案 0.675/0.460，人工标注者 0.722/0.633，
Vocano 约 0.64/0.50，Basic Pitch 0.561/0.398。**no-offset 已超过 Vocano**，剩余差距全在止音
（本方案缺口 0.215、Vocano 约 0.14、人类 0.089）。0.50 观察目标就是 Vocano 的发表值，
当前距其 0.04（全集，部分在样本内）至 0.06（留出，样本外）。GuitarSet 的绝对分（0.902/0.790）**不可用**——YourMT3 训练混合
